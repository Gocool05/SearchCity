...
const VendorFeed = async() =>{
  const response = await axios.get(`${API_URL}/api/vendor-posts?populate=*`)
  return response.data.data
}
const {data:vendorpost,isLoading,error} =useQuery("VendorFeed", VendorFeed)
console.log(vendorpost,'vendorpost')

  return (
    <div className="flex flex-col h-screen">
  {/* Navbar */}
  <Navbar />
    <main className="self-stretch h-[1267px] flex shrink-0 [debug_commit:1de1738] max-w-full text-left text-lg text-gray-200 font-base-regular lg:h-auto lg:min-h-[1267]">
    <SideBar />
    <div className='w-full '>
       ...
       {vendorpost && vendorpost.map((vendor,index)=>(
        <div key={index} className="absolute top-[303px] left-[318px] shadow-[0px_20px_30px_rgba(0,_0,_0,_0.25)] rounded-11xl bg-instagram-light-1 w-[590px] overflow-hidden flex flex-col items-start justify-start pt-9 px-6 pb-8 box-border gap-[30px] max-w-full text-left text-xl text-instagram-light-1 font-base-regular">
              ...
              <div className="self-stretch flex flex-row items-start justify-start gap-[10px]">
                <div className="h-[50px] w-[50px] rounded-81xl bg-gray-500 overflow-hidden shrink-0 flex flex-row items-start justify-start p-[11px] box-border">
                  <div className="relative tracking-[-1px] leading-[140%] inline-block min-w-[28px] mq450:text-base mq450:leading-[22px]">
                    {vendor.attributes.store.data.attributes.StoreName.substring(0,2)}
                  </div>
                </div>
                <div className="flex flex-col items-start justify-start pt-[2.5px] px-0 pb-0 text-lg text-black font-base-regular">
                  <div className="flex flex-col items-start justify-start">
                    ...
                    {vendor.attributes.store.data.attributes.StoreName}
                    </div>
        ...
        <div className="self-stretch h-[520px] rounded-11xl bg-gray-400 overflow-hidden shrink-0 flex flex-col items-start justify-start pt-[3px] px-0 pb-0 box-border relative max-w-full">
          <div className="ml-[-117px] w-[776px] flex-1 flex flex-row items-end justify-center py-[18px] px-5 box-border bg-[url('https://res.cloudinary.com/dx78kzenz/image/upload/v1710237829/Grunge-Effect-Black-Wallpaper-for-Website_fhprrb.jpg')] bg-cover bg-no-repeat bg-[top] max-w-[144%] shrink-0">
            <img
              ...
              alt=""
              src={`${API_URL}${vendor.attributes.upload.data[0].attributes.url}`}
            />
